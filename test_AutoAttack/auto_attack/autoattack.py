...
class AutoAttack():
    def __init__(self, model, norm='Linf', eps=.3, seed=None, verbose=True,
                 ...
                 logger=None, device='cuda', log_path=None):
        self.model = model
        self.norm = norm
        ...
        with torch.no_grad():
            ...
            for attack in self.attacks_to_run:
                ...
                for batch_idx in range(n_batches):
                    ...
                    output, _ = self.get_logits(adv_curr)
                    ...
                    if self.verbose:
                        ...
                        print('{%s} - {%d}/{%d} - {%d} out of {%d} successfully perturbed' % (attack, batch_idx + 1, n_batches, num_non_robust_batch, x.shape[0]))


                robust_accuracy = torch.sum(robust_flags).item() / x_orig.shape[0]
                if self.verbose:
                    print('robust accuracy after {%s}: {%f} (total time {%f} )' % (attack.upper(), robust_accuracy, time.time() - startt))
            ...
            if self.verbose:
                ...
                print('max {%s} perturbation: {%f}, nan in tensor: {%d}, max: {%f}, min: {%f}' % (self.norm, res.max(), (x_adv != x_adv).sum(), x_adv.max(), x_adv.min()))
                print('robust accuracy: {%f}' % (robust_accuracy))
